...
local command = {
    ...
    GETGLOBAL = function(skt, param)
        if hal.globals[param] == nil then
            ...
            local line = string.sub(string.format("%q", tostring(hal.globals[param])),2,-2)
            sendbody(skt, {line})
        end
        return true -- return nil plus error to indicate error and close connection
    end,
    ...
    LISTGLOBALS = function(skt, param)
        skt:send(response[231])     -- list follows
        local lines
        for k,v in pairs(hal.globals) do
